...
export function usePushNotifications({
    ...
    // Register (once per token) whenever a session begins. Resetting the guard on
    // sign-out means the next sign-in re-registers, re-associating a shared
    // device's token with the new user server-side.
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                registerToken.mutate(
                    ...
                    {
                        onSuccess: () => void saveRegisteredPushToken(token),
                        onError: () => {
                            // Allow a retry on the next render/session.
                            registeredTokenRef.current = null;
                        },
                    }
                );
